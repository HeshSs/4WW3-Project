...
while ($row = $getTable->fetch()) {
    // Get row data
    $row_data = $row['SpotName'] . ", " . $row['SpotLatitude'] . ", " . $row['SpotLongitude'] . ", " . $row['SpotType'] . ", " . $row['SpotDescription'];
...
if (htmlspecialchars($_GET["table"]) === "spots") {
    ...
    if (substr($query, 0, 1) == "(") {
        ...
        foreach ($arr as &$row) {
            ...
            $inLat = abs(floatval($row_data[1]) - floatval($pair[0])) < 0.00001;
            $inLong = abs(floatval($row_data[2]) - floatval($pair[1])) < 0.00001;
            if ($inLat or $inLong) {
